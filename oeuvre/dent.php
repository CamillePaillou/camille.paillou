<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Camille Paillou </title>
    <link rel="stylesheet" href="../css/cpaillou.css"/>
    <link rel="stylesheet" href="../css/normalize.css" />
    <script src="/js/typing3.js" defer></script>
</head>
<body>

<?php require_once 'partials/headertitreart.php'; ?>

<?php require_once 'partials/indexart.php'; ?>

<?php require_once 'partials/menuart.php'; ?>

    <div class="dev-container">
        <div class="flecheprev-dev"><a href="rouleau.html">&larr;</a></div>
        <div class="flechesuiv-dev"><a href="echine.html">&rarr;</a></div>
    </div>
    <div class="colonne-dev">
        <div class="dev-image">
            <img src="/images/art/dent/1.webp"> 
            <img src="/images/art/dent/2.webp">
            <img src="/images/art/dent/3.webp">
        </div>
        <div class="dev-p">
            <p><strong>Dent</strong></p>
            <p>2025<br>Papier mâché, cire</p>
        </div>
    </div>

    <?php require_once 'partials/footer.php'; ?>

</body>
</html>